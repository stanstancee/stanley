...
useEffect(() => {
  ...
  return (
    ...
    <div className="contact" style={{backgroundColor:"hsl(0, 0%, 97%)",color:"hsl(255, 11%, 22%)"}}>
...
{  submitted && counter>1 ?<Success counter={counter} />:
   ...
   <div className="a-details">
   <label htmlFor="name" required><span><Name className="para i"/> </span> Name </label>
   <input type='text' onChange={handleChange} name="name" value={contact.name} /><br />
   </div>

   <div className="a-details" >
   <label htmlFor="email" required><span><Mail className="para i" /> </span> Email </label>
   <input type='email' onChange={handleChange}name="email" value={contact.email} /><br />
   </div>
 
 <div className="a-details">
 <label htmlFor="phone"><span><Phone className="para i" /> </span> Phone </label>
   <input type='tel' onChange={handleChange} name="phone" value={contact.phone} />   <br />
 </div>

<div className="a-details">
<label  className="col-10" htmlFor="message"> <span><Message className="para i"/></span>Message</label>
 <textarea cols="60" rows="5" onChange={handleChange} name="message" value={contact.message} ></textarea>

</div>
  <div >
  <Button  type="submit"  icon={true} name={loading?"Loading...":"Submit"} />
  </div>
   ...
   <Question message="Take a look at my projects" name="portfolio" link="/Portfolio"  />
    </div>
  );
}
